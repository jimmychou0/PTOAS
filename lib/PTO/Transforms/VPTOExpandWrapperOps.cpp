...
static FailureOr<Value> packMadXt(Location loc, const MadXtConfig &config,
  ...
  auto coerceToI64 = [&](Value value) -> Value {
    ...
    // PTODSL scalar.select); arith ops require signless operands, so reinterpret
    // the bits first and then widen.
  ...
  if (config.unitFlagValue) {
    // Runtime unit_flag operand: frontends pass 0/2/3 per the CCE contract.
